...
from app.agents.digital_asset_generator import generate_content_for_editing, assemble_and_deploy_asset
...
class AssemblyRequest(BaseModel):
    ...
    # Phase 4 (Polish): Add optional site_id for re-deployment
...
def run_deployment_task(task_id: str, user_id: str, request_data: dict):
    """A wrapper function that runs the assembly and deployment agent."""
    try:
        user_doc = db.collection('users').document(user_id).get()
        netlify_key = user_doc.to_dict().get('netlify_api_key') if user_doc.exists else None
        
        result = assemble_and_deploy_asset(
            ...
            netlify_api_key=netlify_key,
            # Pass the existing site_id if it's a re-deploy
            site_id=request_data.get('netlify_site_id') 
        )
        
        # If deployment was successful, save/update the site info in Firestore
        if result.get("success") and result.get("deployment_result", {}).get("success"):
            dep_res = result["deployment_result"]
            site_doc_ref = db.collection('users').document(user_id).collection('sites').document(dep_res['site_id'])
            site_doc_ref.set({
                "business_name": request_data['business_name'],
                "niche": request_data['niche'],
                "location": request_data['location'],
                "netlify_site_id": dep_res['site_id'],
                "netlify_url": dep_res['url'],
                "content": request_data['edited_content'],
                "site_structure": request_data['site_structure'],
                "last_updated": firestore.SERVER_TIMESTAMP
            }, merge=True)

        deployment_tasks[task_id] = {"status": "complete", "result": result}
    except Exception as e:
        deployment_tasks[task_id] = {"status": "failed", "error": str(e)}
...
def start_assembly_and_deployment(
    request: AssemblyRequest,
    background_tasks: BackgroundTasks,
    user: dict = Depends(get_current_user)
):
    """Starts a background task to assemble the site with edited content and deploy it."""
    user_id = user["uid"]
    task_id = str(uuid.uuid4())
    deployment_tasks[task_id] = {"status": "in_progress"}
    
    background_tasks.add_task(run_deployment_task, task_id, user_id, request.dict())
    
    return {"message": "Assembly and deployment started.", "task_id": task_id}

@app.get("/api/v1/deployment-status/{task_id}")
def get_deployment_status(task_id: str, user: dict = Depends(get_current_user)):
    """Polls for the status of a background deployment task."""
