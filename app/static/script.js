...
// Version: 2.2 (Enhanced Error Handling)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
        ...
        storageBucket: "local-arbitrage-mcp.appspot.com",
        ...
        if (e.target) {
            if (e.target.classList.contains('generate-content-btn')) {
                ...
                if (siteId && siteId !== 'undefined') {
                    ...
                    assetStatus.innerHTML = `<p class="error-message">Error: Site ID is missing. Please refresh and try again.</p>`;
                }
            }
        }
    }
    ...
    function closeContentEditor() {
        ...
        editorState = {}; // Clear state on close
    }
    ...
    async function handleFinalizeDeploy() {
        // Save the content of the currently active tab before submitting
        const activeTab = editorTabs.querySelector('.tab.active');
        if (activeTab) {
            editorState.content[activeTab.dataset.filename] = contentEditorTextarea.value;
        }
        ...
        // *** THIS IS THE FIX ***
        // We capture the business name before it can be cleared.
        const businessNameForPolling = editorState.businessName;

        assetStatus.innerHTML = `<p>Pushing content to CMS for "${businessNameForPolling}"...</p>`;
        
        try {
            // First, send the request with the complete editorState data.
            ...
            // Now that the request is sent, we can close the editor.
            closeContentEditor();
            
            const data = await response.json();
            if (!response.ok) {
                throw new Error(JSON.stringify(data.detail, null, 2) || 'Failed to start task.');
            }
            
            assetStatus.innerHTML = `<p>CMS push for "${businessNameForPolling}" is in progress. Task ID: ${data.task_id}</p>`;
            // Pass the captured name to the polling function.
            pollDeploymentStatus(data.task_id, businessNameForPolling);
        } catch (error) {
            // If the fetch itself fails, we still need to close the editor.
            closeContentEditor();
        ...
        const intervalId = setInterval(async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    assetStatus.innerHTML = `<p class="error-message">Could not get task status. It may have expired.</p>`;
                    return; 
                }
                const data = await response.json();
                ...
                clearInterval(intervalId);
                assetStatus.innerHTML = `<p class="error-message">Error polling for status: ${error.message}</p>`;
            }
        }, 5000);
    }

    function pageNameFromFilename(filename) { return filename.split('.')[0].replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()); }
});
